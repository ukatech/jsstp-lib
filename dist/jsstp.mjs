...
class ghost_events_queryer_t {
	 ...
	 */
	async check_event(event_name, security_level = default_security_level) {
...
class sstp_info_t extends base_sstp_info_t {
	...
	constructor(info_head, info_body, unknown_lines = {}) {
		...
		return new Proxy(this, {
			get: new_get_handler({
				string_key_handler: (target, key) => x_sstp_passthru_head + key in target ? target.get_passthrough(key) : undefined$1
			})
		});
	}
	/**
...
class jsstp_t {
	 ...
	 * }
	 */
	async has_event(event_name, security_level = default_security_level) {
