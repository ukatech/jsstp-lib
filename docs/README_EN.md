...
//queryer, like jsstp, checks events with optional arguments specifying the event's security level; the default security level varies with the environment in which jsstp is running:
...
// If you want to fix the query local event, you need to specify the security level as "local", like this:
...
if (!queryer.fast_query_available)
	console.info("Can't get a list of supported events fast");// This won't affect usage, it will just cause a query request to be made for uncached events: if ghost supports `Get_Supported_Events` events the queryer will use it to get a list of events (which will be much faster!)
else
	console.info("ok oh");
